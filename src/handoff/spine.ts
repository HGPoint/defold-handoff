...
import { GUI_EXPORT_PIPELINE, GUI_SPINES_EXPORT_PIPELINE } from "utilities/gui";
...
export async function exportGUISpines(layers: Exclude<ExportableLayer, SliceLayer>[]): Promise<BundleData> {
  const data = packGUI(layers, false, true);
  ...
  const serializedGUIAtlasData = await exportGUIAtlases(layers, true, false);
